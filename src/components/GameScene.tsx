...
import { inputManager } from '../store/inputStore';
...
export function GameScene() {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'a' || e.key === 'A' || e.key === 'ArrowLeft') {
        inputManager.turnLeft = true;
        inputManager.targetAngle = null;
        inputManager.lastInputDevice = 'keyboard';
      }
      if (e.key === 'd' || e.key === 'D' || e.key === 'ArrowRight') {
        inputManager.turnRight = true;
        inputManager.targetAngle = null;
        inputManager.lastInputDevice = 'keyboard';
      }
      if (e.key === ' ' || e.key === 'w' || e.key === 'W' || e.key === 'ArrowUp') {
        inputManager.keyBoost = true;
    ...
    const handleKeyUp = (e: KeyboardEvent) => {
      ...
      if (e.key === ' ' || e.key === 'w' || e.key === 'W' || e.key === 'ArrowUp') {
        inputManager.keyBoost = false;
        inputManager.updateBoost();
      }
    };

    // Mouse handlers for steering & boosting
    const handleMouseMove = (e: MouseEvent) => {
      ...
      inputManager.mousePos = { x: e.clientX, y: e.clientY };
      inputManager.mouseActive = true;
      inputManager.lastInputDevice = 'mouse';
    };

    const handleMouseDown = (e: MouseEvent) => {
      // Don't boost if clicking on an interactive UI element
      const target = e.target as HTMLElement;
      if (target.closest('button') || target.closest('a') || target.closest('.pointer-events-auto:not(#touch-surface)')) {
        return;
      }
      if (e.button === 0 || e.button === 2) {
        inputManager.mouseBoost = true;
        inputManager.updateBoost();
        inputManager.lastInputDevice = 'mouse';
      }
    };
    ...
    const handleMouseUp = (e: MouseEvent) => {
      if (e.button === 0 || e.button === 2) {
        inputManager.mouseBoost = false;
        inputManager.updateBoost();
      }
    };

    const handleContextMenu = (e: MouseEvent) => {
      // Prevent context menu during gameplay so right-click can boost smoothly
  ...
  useFrame((state, delta) => {
    ...
    if (serverPlayer && serverPlayer.state === 'alive') {
      ...
      // Turning mechanics: priority given to direct keyboard, then to mouse/touch targetAngle
      if (inputManager.turnLeft) {
        localPlayerRef.current.currentAngle += TURN_SPEED * delta;
      } else if (inputManager.turnRight) {
        localPlayerRef.current.currentAngle -= TURN_SPEED * delta;
      } else if (inputManager.targetAngle !== null) {
        let diff = inputManager.targetAngle - localPlayerRef.current.currentAngle;
        // Wrap diff to [-PI, PI]
        diff = Math.atan2(Math.sin(diff), Math.cos(diff));
        const maxTurn = TURN_SPEED * delta;
        if (Math.abs(diff) <= maxTurn) {
          localPlayerRef.current.currentAngle = inputManager.targetAngle;
        } else {
      ...
      // Boosting mechanics
      const wantBoost = inputManager.isBoosting && localPlayerRef.current.score > 10;
      localPlayerRef.current.isBoosting = wantBoost;
      const speed = wantBoost ? BOOST_SPEED : BASE_SPEED;
      ...
      // Dynamic responsive camera distance calculation
      // For mobile portrait screens (aspect ratio < 1), zoom out dynamically so horizontal view is wide enough
      const aspect = size.width / Math.max(1, size.height);
      let baseZ = 20 + localPlayerRef.current.score * 0.2;
      if (aspect < 1) {
        baseZ = baseZ / Math.max(0.55, aspect);
      }
      const targetZ = Math.min(65, Math.max(20, baseZ));
